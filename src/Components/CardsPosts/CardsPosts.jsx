...
export default function CardsPosts({ userPosts, id }) {
  ...
  function getAllComments() {
    return axios.get(
      `${import.meta.env.VITE_BASE_URL}/posts/${idPost}/comments?page=1&limit=10`,
      { headers: { token: localStorage.getItem("token") } },
    );
  }

  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["allComments", idPost],
  ...
  const { mutate: handleLike } = useMutation({
    mutationFn: () => {
      ...
      const promise = axios.put(
        `${import.meta.env.VITE_BASE_URL}/posts/${idPost}/like`,
        {},
        { headers: { token: localStorage.getItem("token") } },
      );
      return promise;
  ...
  const { mutate: handleBookmark } = useMutation({
    mutationFn: () => {
      ...
      const promise = axios.put(
        `${import.meta.env.VITE_BASE_URL}/posts/${idPost}/bookmark`,
        {},
        { headers: { token: localStorage.getItem("token") } },
      );
      ...
      <CardBody className="flex flex-col px-0 py-0 overflow-visible">
        {updatePost ? (
          <Form
            ...
            onSubmit={handleSubmit((data) => mutate({ data, postId: idPost }))}
          >
          ...
          >
            {updateLike ? (
              ...
              <>
                ...
                />
                <span
                  className={`text-[15px] ${isLiked ? "text-red-500" : "text-gray-600"}`}
                >
                  {likesCount > 0 && (
